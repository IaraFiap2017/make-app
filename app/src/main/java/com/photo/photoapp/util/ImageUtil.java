...
public class ImageUtil {
    ...
    public static Bitmap loadSizeLimitedBitmap (String mCurrentPhotoPath) throws IOException {

        // Load the image into InputStream.
        InputStream fileInputStream = new FileInputStream(mCurrentPhotoPath);

        // For saving memory, only decode the image meta and get the side length.
        BitmapFactory.Options options = new BitmapFactory.Options();
        options.inJustDecodeBounds = true;
        Rect outPadding = new Rect();
        BitmapFactory.decodeStream(fileInputStream, outPadding, options);
        ...
        Bitmap bitmap = BitmapFactory.decodeStream(fileInputStream, outPadding, options);
    ...
    public static int getImageRotationAngle(Uri imageUri, ContentResolver contentResolver) throws IOException {
        int angle = 0;
